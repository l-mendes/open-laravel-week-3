            ...
            <div class="mx-auto max-w-2xl px-4 lg:max-w-none lg:px-0">
                ...
                <x-checkout.product-list>
                    @foreach ($cart['skus'] as $sku)
                        <x-checkout.product-item :name="$sku['name']" :price="$sku['price']" :quantity="$sku['pivot']['quantity']" :features="collect($sku['features'])->map(
                    ...
                    <div class="flex flex-row items-center text-white">
                        ...
                        <nav class="flex mb-4" aria-label="Breadcrumb">
                            <ol class="text-xs inline-flex items-center space-x-1 md:space-x-3">
                                <li @class([
                                    'font-bold' => $step === CheckoutStepsEnum::INFORMATION->value,
                                ])>
                                ...
                                <li @class([
                                    ...
                                    'font-bold' => $step === CheckoutStepsEnum::SHIPPING->value,
                                ])>
                                ...
                                <li @class([
                                    'inline-flex items-center',
                                    'font-bold' => $step === CheckoutStepsEnum::PAYMENT->value,
                                ])>
                                    <svg class="w-3 h-3 text-gray-400 mx-1" aria-hidden="true"
                                        xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10">
                                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
